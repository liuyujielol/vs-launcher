        ...
        <ul className="w-full flex flex-col">
          {config.gameVersions.length < 1 && (
            <div className="w-full flex flex-col items-center justify-center gap-2 rounded bg-zinc-850 p-4">
              <h2 className="text-2xl">{t("features.versions.noVersionsFound")}</h2>
          ...
          {config.gameVersions.map((gv) => (
            <li key={gv.version} className={`w-full px-2 py-1 hover:pl-3 hover:pr-1 duration-100 odd:bg-zinc-850 rounded cursor-pointer group`}>
              <div className="flex justify-between items-center">
                <span>{gv.version}</span>
                <span className="hidden group-hover:block text-sm text-zinc-500">{gv.path}</span>
                <div className="flex gap-2">
                  ...
                  <Button
                    onClick={async () => {
                      if (!(await window.api.pathsManager.checkPathExists(gv.path))) {
                        addNotification(t("notifications.titles.error"), t("notifications.body.folderDoesntExists"), "error")
                      } else {
        ...
        {versionToDelete !== null && (
          ...
          >
            <motion.div initial={{ opacity: 0, scale: 0 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0 }}>
              <DialogPanel className="flex flex-col gap-4 text-center bg-zinc-850 rounded p-8">
